...
class DatabaseService {
  ...
  async getInvestors() {
    return await this.supabaseDb.getInvestors();
  }

  async getExpenses() {
    return await this.supabaseDb.getExpenses();
  }

  async getRevenues() {
    return await this.supabaseDb.getRevenues();
  }

  async getWithdrawals() {
    return await this.supabaseDb.getWithdrawals();
  }
  ...
  async getDashboardStats(customSettings?: any) {
    ...
    // الرصيد المتاح للتوزيع = صافي الربح - نسبة المشروع - التخصيصات الإضافية - سحوبات المشروع - سحوبات المستثمرين
